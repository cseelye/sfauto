...
"""
This action will get an advanced configuration property of a VM
...
class VmwareGetVmPropertyAction(ActionBase):
    class Events:
    ...
    def Get(self, vm_name, prop_name, mgmt_server=sfdefaults.fc_mgmt_server, mgmt_user=sfdefaults.fc_vsphere_user, mgmt_pass=sfdefaults.fc_vsphere_pass, bash=False, csv=False, debug=False):
        """
        Get an advanced property
        """
        ...
        try:
            with libvmware.VsphereConnection(mgmt_server, mgmt_user, mgmt_pass) as vsphere:
                ...
                result = libvmware.GetVMXProperty(vsphere, vm, prop_name)
    ...
    def Execute(self, vm_name, prop_name, mgmt_server=sfdefaults.fc_mgmt_server, mgmt_user=sfdefaults.fc_vsphere_user, mgmt_pass=sfdefaults.fc_vsphere_pass, bash=False, csv=False, debug=False):
        """
        Get an advanced property
        """
        del self
        ...
        if csv or bash:
            ...
            mylog.info('{}:   {} = {}'.format(vm_name, prop_name, result))
        return True
...
if __name__ == '__main__':
    ...
    parser.add_option("--prop_name", type="string", dest="prop_name", default=None, help="the name of property to set")
    ...
    try:
        ...
        if Execute(vm_name=options.vm_name, prop_name=options.prop_name, mgmt_server=options.mgmt_server, mgmt_user=options.mgmt_user, mgmt_pass=options.mgmt_pass, bash=options.bash, csv=options.csv, debug=options.debug):
            sys.exit(0)
        else:
            sys.exit(1)
