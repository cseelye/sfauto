"""
This action will preform a master reboot stress test
...
When run as a script, the following options/env variables apply:
    ...
    --iteration         how many times to reboot the master node, 0=forever
"""
...
class StressRebootMasterAction(ActionBase):
    class Events:
        """
    ...
    def fail(self, message, emailTo):
        mylog.error(message)
        send_email.Execute(emailTo=emailTo, emailSubject="Failed Stress Reboot Master", emailBody=message)
    ...
    def Execute(self, mvip=sfdefaults.mvip, username=sfdefaults.username, password=sfdefaults.password, clientIPs=None, clientUser=sfdefaults.client_user, clientPass=sfdefaults.client_pass, waitTime=300, emailTo=None, addSSH=False, bsCheck=True, debug=False, iteration=1):
        ...
        send_email.Execute(emailTo=emailTo, emailSubject=mvip+ ": Finished Master Node Reboot in " + delta_time, emailBody=emailBody)
        ...
        mylog.passed("The Stress Master Reboot Test has passed")
