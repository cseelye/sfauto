...
"""
This action will set the advanced configuration properties on a VM
...
class VmwareSetVmPropertyAction(ActionBase):
    class Events:
    ...
    def Execute(self, vm_name, prop_name, prop_val, mgmt_server=sfdefaults.fc_mgmt_server, mgmt_user=sfdefaults.fc_vsphere_user, mgmt_pass=sfdefaults.fc_vsphere_pass, bash=False, csv=False, debug=False):
        """
        ...
        try:
            with libvmware.VsphereConnection(mgmt_server, mgmt_user, mgmt_pass) as vsphere:
                ...
                libvmware.SetVMXProperty(vsphere, vm, prop_name, prop_val)
                mylog.passed("Successfully set property on " + vm_name)
...
if __name__ == '__main__':
    ...
    parser.add_option("--prop_val", type="string", dest="prop_val", default=None, help="the value to set")
    ...
    try:
        ...
        if Execute(vm_name=options.vm_name, prop_name=options.prop_name, prop_val=options.prop_val, mgmt_server=options.mgmt_server, mgmt_user=options.mgmt_user, mgmt_pass=options.mgmt_pass, bash=options.bash, csv=options.csv, debug=options.debug):
            sys.exit(0)
        else:
            sys.exit(1)
