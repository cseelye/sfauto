...
class ExecuteClientCommandAction(ActionBase):
    ...
    def Execute(self, client_ip, command, csv=False, bash=False, client_user=sfdefaults.client_user, client_pass=sfdefaults.client_pass, debug=False):
        """
...
if __name__ == '__main__':
    ...
    parser.add_option("--command", type="string", dest="command", default=None, help="the command to execute on the client")
    ...
    try:
        ...
        sys.exit( Execute(options.client_ip, options.command, options.csv, options.bash, options.client_user, options.client_pass, options.debug) )
