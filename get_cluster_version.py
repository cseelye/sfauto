...
"""
This action will show the software version of the cluster
...
class GetClusterVersionAction(ActionBase):
    class Events:
        """
    ...
    def Get(self, mvip, csv=False, bash=False, username=sfdefaults.username, password=sfdefaults.password, debug=False):
        ...
        version_details = result["clusterVersionInfo"][0]["nodeInternalRevision"]
        fields = version_details.split(" ")
        for field in fields:
    ...
    def Execute(self, mvip, csv=False, bash=False, username=sfdefaults.username, password=sfdefaults.password, debug=False):
        ...
        if bash or csv:
            ...
            mylog.info("Cluster version is " + str(ver))
        return True
