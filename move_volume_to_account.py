...
def main():
    global mvip, username, password, source_account, dest_account, volume_id, volume_name, volume_id, volume_prefix, volume_regex, volume_count
    ...
    parser.add_option("--volume_id", type="int", dest="volume_id", default=volume_id, help="the volume to move")
    ...
    for account in accounts_list["accounts"]:
        if account["username"].lower() == dest_account.lower():
            dest_account_id = account["accountID"]
            break
    if dest_account_id <= 0:
        mylog.error("Could not find account '" + dest_account + "'")
        sys.exit(1)
    ...
    try:
        volumes_to_move = libsf.SearchForVolumes(mvip, username, password, VolumeId=volume_id, VolumeName=volume_name, VolumeRegex=volume_regex, VolumePrefix=volume_prefix, AccountName=source_account, VolumeCount=volume_count)
    except SfError as e:
