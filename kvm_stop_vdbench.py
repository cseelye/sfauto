...
"""
This action will stop vdbench on VMs on a KVM hypervisor
...
When run as a script, the following options/env variables apply:
    ...
    --vm_name           The name of the VM to stop vdbench on

    --vm_regex          Regex to match to select VMs to stop vdbench on

    --vm_count          The number of matching VMs to stop vdbench on
...
class KvmStopVdbenchAction(ActionBase):
    class Events:
        """
    ...
    def Execute(self, vm_name=None, vm_regex=None, vm_count=0, vmhost=sfdefaults.vmhost_kvm, host_user=sfdefaults.host_user, host_pass=sfdefaults.host_pass, client_user=sfdefaults.client_user, client_pass=sfdefaults.client_pass, debug=False):
        """
        Power on VMs
        """
...
if __name__ == '__main__':
    ...
    parser.add_option("--vm_name", type="string", dest="vm_name", default=None, help="the name of the VM to stop vdbench on")
    parser.add_option("--vm_regex", type="string", dest="vm_regex", default=None, help="the regex to match VMs to stop vdbench on")
    ...
    try:
        ...
        if Execute(options.vm_name, options.vm_regex, options.vm_count, options.vmhost, options.host_user, options.host_pass, options.client_user, options.client_pass, options.debug):
            sys.exit(0)
        else:
            sys.exit(1)
