...
class XenDeleteAllSnapshotsAction(ActionBase):
    class Events:
        """
    ...
    def _VmThread(self, vmHost, hostUser, hostPass, vmRecord, vmRef, results, debug):
        ...
        for snap_ref in vmRecord['snapshots']:
            ...
            # Delete the snap
            mylog.info("  " + vm_name + ": Deleting snap " + snap['name_label'])
            try:
                session.xenapi.VM.destroy(snap['ref'])
    ...
    def Execute(self, vm_name=None, vm_regex=None, vm_count=0, vmhost=sfdefaults.vmhost_xen, host_user=sfdefaults.host_user, host_pass=sfdefaults.host_pass, parallel_thresh=sfdefaults.xenapi_parallel_calls_thresh, parallel_max=sfdefaults.xenapi_parallel_calls_max, debug=False):
        ...
        mylog.info(str(len(matched_vms.keys())) + " VMs will have all snapshots deleted: " + ", ".join(matched_vms.keys()))
        ...
        if allgood:
            mylog.passed("Successfully deleted all snaps on all VMs")
            return True
        else:
            mylog.error("Failed to delete all snaps on all VMs")
            return False
...
if __name__ == '__main__':
    ...
    parser.add_option("--vm_name", type="string", dest="vm_name", default=None, help="the name of the VM to delete all snapshots")
    parser.add_option("--vm_regex", type="string", dest="vm_regex", default=None, help="the regex to match names of VMs to delete all snapshots")
    parser.add_option("--vm_count", type="int", dest="vm_count", default=0, help="the number of matching VMs to delete all snapshots (0 to use all)")
