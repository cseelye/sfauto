...
"""
This action will resume VMs to disk
...
class XenResumeVmsAction(ActionBase):
    class Events:
        """
    ...
    def _VmThread(self, vmHost, hostUser, hostPass, vmRecord, vmRef, results, debug):
        ...
        # Resume the VM
        mylog.info("  " + vm_name + ": Resuming VM")
        try:
            session.xenapi.VM.resume(vmRef, False, False)
        except XenAPI.Failure as e:
            mylog.error("  " + vm_name + ": Could not resume VM '" + vm_name + "' - " + str(e))
            self.RaiseFailureEvent(message=str(e), exception=e)
            return

        results[vm_name] = True
        mylog.passed("  " + vm_name + ": Successfully resumed")
    ...
    def Execute(self, vm_name=None, vm_regex=None, vm_count=0, vmhost=sfdefaults.vmhost_xen, host_user=sfdefaults.host_user, host_pass=sfdefaults.host_pass, parallel_thresh=sfdefaults.xenapi_parallel_calls_thresh, parallel_max=sfdefaults.xenapi_parallel_calls_max, debug=False):
        ...
        mylog.info(str(len(matched_vms.keys())) + " VMs will be resumed: " + ", ".join(matched_vms.keys()))
        ...
        if allgood:
            mylog.passed("Successfully resumed all VMs")
            return True
        else:
            mylog.error("Failed to resume all VMs")
            return False
...
if __name__ == '__main__':
    ...
    parser.add_option("--vm_name", type="string", dest="vm_name", default=None, help="the name of the VM to resume")
    parser.add_option("--vm_regex", type="string", dest="vm_regex", default=None, help="the regex to match names of VMs to resume")
    parser.add_option("--vm_count", type="int", dest="vm_count", default=0, help="the number of matching VMs to resume (0 to use all)")
