...
class RemoveIqnFromVolgroupAction(ActionBase):
    ...
    def Execute(self, mvip, iqn, volgroup_name=None, volgroup_id=0, username=sfdefaults.username, password=sfdefaults.password, debug=False):
        """
        Remove an IQN from a volume access group
        """
        ...
        if volgroup_name:
            mylog.info("Removing " + iqn + " to group " + str(volgroup_name))
        else:
            mylog.info("Removing " + iqn + " to group " + str(volgroup_id))
...
if __name__ == '__main__':
    ...
    try:
        ...
        if Execute(options.mvip, options.iqn, options.volgroup_name, options.volgroup_id, options.username, options.password, options.debug):
            sys.exit(0)
        else:
            sys.exit(1)
