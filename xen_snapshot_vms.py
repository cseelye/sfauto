...
"""
This action will snapshot VMs to disk
...
class XenSnapshotVmsAction(ActionBase):
    class Events:
        """
    ...
    def _VmThread(self, vmHost, hostUser, hostPass, vmRecord, vmRef, results, debug):
        ...
        snap_name = vm_name + "-snap"
        snap_number = 1
        for snap_ref in vmRecord['snapshots']:
            snap = session.xenapi.VM.get_record(snap_ref)
            m = re.search(r"snap-*0*(\d+)$", snap["name_label"])
            if m:
                n = int(m.group(1))
                if n >= snap_number:
                    snap_number = n + 1
        snap_name = snap_name + "%05d" % snap_number

        # Snapshot the VM
        mylog.info("  " + vm_name + ": creating snapshot " + snap_name)
        try:
            snap_ref = session.xenapi.VM.snapshot(vmRef, snap_name)
        except XenAPI.Failure as e:
            mylog.error("  " + vm_name + ": Could not snapshot VM - " + str(e))
            self.RaiseFailureEvent(message=str(e), exception=e)
            return

        # Modify the name/description of the snapshot's disks
        try:
            snap = session.xenapi.VM.get_record(snap_ref)
        except XenAPI.Failure as e:
            mylog.error("  " + vm_name + ": Could not get snap record - " + str(e))
            self.RaiseFailureEvent(message=str(e), exception=e)
            return
        allgood = True
        for vbd_ref in snap["VBDs"]:
            ...
            try:
                ...
                continue
            try:
                session.xenapi.VDI.set_name_description(vdi_ref, "Snap [" + snap_name + "] of " + vdi['name_label'])
            except XenAPI.Failure as e:
                ...
                continue
        if not allgood:
            mylog.error("Failed to update description on all snapshot disks")

        results[vm_name] = True
        mylog.passed("  " + vm_name + ": Successfully snapshotted")
    ...
    def Execute(self, vm_name=None, vm_regex=None, vm_count=0, vmhost=sfdefaults.vmhost_xen, host_user=sfdefaults.host_user, host_pass=sfdefaults.host_pass, parallel_thresh=sfdefaults.xenapi_parallel_calls_thresh, parallel_max=sfdefaults.xenapi_parallel_calls_max, debug=False):
        ...
        mylog.info(str(len(matched_vms.keys())) + " VMs will be snapshotted: " + ", ".join(matched_vms.keys()))
        ...
        if allgood:
            mylog.passed("Successfully snapshotted all VMs")
            return True
        else:
...
if __name__ == '__main__':
    ...
    parser.add_option("--vm_name", type="string", dest="vm_name", default=None, help="the name of the VM to snapshot")
    parser.add_option("--vm_regex", type="string", dest="vm_regex", default=None, help="the regex to match names of VMs to snapshot")
    parser.add_option("--vm_count", type="int", dest="vm_count", default=0, help="the number of matching VMs to snapshot (0 to use all)")
