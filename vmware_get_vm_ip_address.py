...
"""
This action will get the IP address of a VM
...
class VmwareGetVmIpAddressAction(ActionBase):
    class Events:
        """
                ...
                if vm.guest.net:
                    for nic in vm.guest.net:
                        if nic.ipAddress:
                            ...
                            break
                if not ip:
                    mylog.error('Could not find an IP address for VM {}'.format(vm_name))
                    return False
                return ip
    ...
    def Execute(self, vm_name, mgmt_server=sfdefaults.fc_mgmt_server, mgmt_user=sfdefaults.fc_vsphere_user, mgmt_pass=sfdefaults.fc_vsphere_pass, bash=False, csv=False, debug=False):
        ...
        if csv or bash:
            ...
            mylog.info('{} has IP address {}'.format(vm_name, ip_addr))
        return True
