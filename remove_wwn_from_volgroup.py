...
"""
This action will remove a Fc WWN from a volume access group
...
When run as a script, the following options/env variables apply:
    ...
    --wwn               The WWN to remove from the group
...
class RemoveWwnFromVolgroupAction(ActionBase):
    class Events:
        """
    ...
    def Execute(self, wwn=None, mvip=sfdefaults.mvip, volgroup_name=None, volgroup_id=0, username=sfdefaults.username, password=sfdefaults.password, debug=False):
        """
        Remove the specified WWN from the specified volume access group
        """
        ...
        if normalized_wwn not in full_wwn_list:
        ...
        mylog.passed("Successfully removed WWN from group")
        return True
...
if __name__ == '__main__':
    ...
    parser.add_option("--wwn", type="string", dest="wwn", default=None, help="WWN to remove from the group")
