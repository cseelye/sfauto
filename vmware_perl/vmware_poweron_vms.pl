...
my %opts = (
    ...
    wait_for_booted => {
        type => "",
        help => "Wait for the VMs to boot up",
        required => 0,
    },
    result_address => {
        type => "=s",
...
my $wait_for_booted = Opts::get_option('wait_for_booted');
...
{
    my $th = threads->create(\&poweronVM, $vm_mor);
...
sub poweronVM
{
    ...
    eval
    {
        libvmware::WaitForTask(vim => $threadvim, task_ref => $task_ref, fail_message => "  Failed to power on " . $vm->name);
    };
    if ($@)
    {
        my $er = $@;
