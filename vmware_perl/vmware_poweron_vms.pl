...
my %opts = (
    ...
    vm_regex => {
        type => "=s",
        help => "The regex to match names of virtual machines to power on",
        required => 0,
    },
    vm_count => {
        type => "=s",
...
eval
{
    @vm_list = libvmware::SearchForVms(datacenter_name => $dc_name, cluster_name => $cluster_name, pool_name => $pool_name, folder_name => $folder_name, recurse => $recurse, vm_name => $vm_name, vm_regex => $vm_regex, vm_count => $vm_count, vm_powerstate => "poweredOff");
    if (scalar(@vm_list) <= 0)
    {
...
foreach my $vm_mor (@vm_list)
{
    ...
    if ($@)
    {
        libvmware::DisplayFault("Failed to power on VM", $@);
        $error++;
    }
}
