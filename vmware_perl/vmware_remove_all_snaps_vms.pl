...
my %opts = (
    ...
    vm_name => {
        type => "=s",
        help => "The name of the virtual machine to remove all snapshots",
        required => 0,
    },
    vm_regex => {
        type => "=s",
        help => "The regex to match names of virtual machines to remove all snapshots",
        required => 0,
    },
    vm_count => {
        type => "=i",
        help => "The number of matching virtual machines to remove all snapshots",
        required => 0,
    },
    result_address => {
        type => "=s",
...
mylog::info("Found " . scalar(@vm_list) . " matching VMs to remove all snapshots from");
...
{
    mylog::pass("Successfully remove all snapshots from all VMs");
}
else
{
    mylog::error("Failed to remove snapshots from all VMs");
    $exitcode = 1;
}
...
sub revertVM
{
    ...
    mylog::info("  " . $vm->name . ": Removing all snapshot");
    my $task_ref;
    eval
    {
        $task_ref = $vm->RemoveAllSnapshots_Task();
    };
    if ($@)
    {
        libvmware::DisplayFault("  " . $vm->name . ": Failed to remove snapshots", $@);
        return 0;
    }

    eval
    {
        libvmware::WaitForTask(vim => $threadvim, task_ref => $task_ref, fail_message => "  " . $vm->name . ": Failed to remove snapshots");
    };
    if ($@)
    {
        my $er = $@;
        $er =~ s/\s+$//;
        mylog::error("  " . $vm->name . ": Failed to remove all snapshots - $er");
        return 0;
    }
    ...
    mylog::pass("  " . $vm->name . ": Sucessfully removed snapshots");
    return 1;
}
