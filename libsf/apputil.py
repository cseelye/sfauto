...
from . import logutil, sfdefaults
from . import threadutil
from .util import InvalidArgumentError
...
class PythonApp(object):
    ...
    def __init__(self, main_func, options=None, timer=True, require_superuser=False, custom_log_config=None):
        ...
        if not callable(main_func):
            raise TypeError("mainFunc must be callable")
        self.main = main_func
        ...
        sfdefaults.parallel_calls_max = self.PopOption("parallel_max")
        sfdefaults.parallel_calls_min = self.PopOption("parallel_min")
    ...
    def Run(self, *args, **kwargs):
        ...
        self.log.debug("Starting " + str(sys.argv))
        self.startTime = time.time()
