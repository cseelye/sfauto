...
class KvmRemoveVmsAction(ActionBase):
    class Events:
        """
    ...
    def _VmThreadDelete(self, vmHost, hostUser, hostPass, connection, vm, results):
        #connect again
        ...
        try:
            ...
            mylog.error("Unable to delete " + vm)
            conn.close()
            return

        mylog.info("Deleting: " + vm.name())
        try:
            vm.undefine()
            mylog.passed("Successfully deleted " + vm.name())
            results[vm.name()] = True
        except libvirt.libvirtError as e:
            mylog.error("Failed to delete " + vm.name() + ": " + str(e))

        #close when done
        conn.close()


    def _VmThreadPowerOffAndDelete(self, vmHost, hostUser, hostPass, connection, vm, results):
        #connect again
        mylog.info("Connecting to " + vm)
        try:
            if connection == "ssh":
        ...
        try:
            ...
            mylog.error("Unable to power off and delete " + vm)
            conn.close()
            return
        ...
        if vm_names != None:
            ...
            for name in vm_names:
                ...
                if state == libvirt.VIR_DOMAIN_RUNNING:
                    th = multiprocessing.Process(target=self._VmThreadPowerOffAndDelete, args=(vmhost, host_user, host_pass, connection, vm.name(), results))
                    th.daemon = True
                    self._threads.append(th)
                else:
                    results[vm.name()] = False
                    th = multiprocessing.Process(target=self._VmThreadDelete, args=(vmhost, host_user, host_pass, connection, vm.name(), results))
                    th.daemon = True
                    self._threads.append(th)
