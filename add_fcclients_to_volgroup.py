...
The action connects to each specified client, queries the FC WWNs, then adds those WWNs to the volume access group on the cluster
...
class AddFcclientsToVolgroupAction(ActionBase):
    ...
    def Execute(self, mvip=sfdefaults.mvip, client_ips=None, volgroup_name=None, volgroup_id=0, username=sfdefaults.username, password=sfdefaults.password, client_user=sfdefaults.client_user, client_pass=sfdefaults.client_pass, debug=False):
        ...
        for wwn in new_wwns:
            if wwn in full_wwn_list:
                mylog.debug(wwn + " is already in the volume group")
                continue
            full_wwn_list.append(wwn)
        all_init_list = volgroup["iscsiInitiators"] + full_wwn_list

        # Add the WWNs to the volume group
        mylog.info("Adding client WWNs to group")
        params = {}
