...
"""
This action will power off VMs on a XenServer hypervisor
    ...
    def Execute(self, vm_name=None, vm_regex=None, vm_count=0, vmhost=sfdefaults.vmhost_xen, host_user=sfdefaults.host_user, host_pass=sfdefaults.host_pass, debug=False):
        ...
        for vname in sorted(matched_vms.keys()):
            ...
            if vm["power_state"] == "Halted":
                ...
                mylog.info("  Powering off " + vm["name_label"])
                try:
                    session.xenapi.VM.hard_shutdown(vm_ref)
                    power_count += 1
                    mylog.passed("  Successfully powered off " + vname)
                except XenAPI.Failure as e:
                    mylog.error("  Failed to power off " + vm["name_label"] + " - " + str(e))
                    self.RaiseFailureEvent(message=str(e), exception=e)
