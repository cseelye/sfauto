...
"""
This action will set the amount of memory on a VM
...
class VmwareSetVmMemoryAction(ActionBase):
    class Events:
        """
    ...
    def ValidateArgs(self, args):
        libsf.ValidateArgs({"mgmt_server" : libsf.IsValidIpv4Address,
                            ...
                            "mgmt_pass" : None,
                            "vm_name" : None,
                            "memory" : libsf.IsPositiveInteger},
            args)

    def Execute(self, vm_name, memory, mgmt_server=sfdefaults.fc_mgmt_server, mgmt_user=sfdefaults.fc_vsphere_user, mgmt_pass=sfdefaults.fc_vsphere_pass, bash=False, csv=False, debug=False):
        """
        Set the VM memory
        """
...
if __name__ == '__main__':
    ...
    parser.add_option("--mem", type="int", dest="mem", default=None, help="the amount of memory to set, in GB")
    ...
    try:
        ...
        if Execute(vm_name=options.vm_name, memory=options.mem, mgmt_server=options.mgmt_server, mgmt_user=options.mgmt_user, mgmt_pass=options.mgmt_pass, bash=options.bash, csv=options.csv, debug=options.debug):
            sys.exit(0)
        else:
            sys.exit(1)
