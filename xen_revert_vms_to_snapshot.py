...
"""
This action will revert VMs to the latest snapshot
...
class XenRevertVmsToSnapshotAction(ActionBase):
    class Events:
        """
    ...
    def _VmThread(self, vmHost, hostUser, hostPass, vmRecord, vmRef, results, debug):
        ...
        # Revert to the snap
        mylog.info("  " + vm_name + ": Reverting to snap " + newest_snap['name_label'])
        try:
            session.xenapi.VM.revert(newest_snap['ref'])
        except XenAPI.Failure as e:
            mylog.error("  " + vm_name + ": Could not revert to snapshot " + snap['name_label'] + " - " + str(e))
            self.RaiseFailureEvent(message=str(e), exception=e)
            return

        results[vm_name] = True
        mylog.passed("  " + vm_name + ": Successfully reverted to snapshot")
    ...
    def Execute(self, vm_name=None, vm_regex=None, vm_count=0, vmhost=sfdefaults.vmhost_xen, host_user=sfdefaults.host_user, host_pass=sfdefaults.host_pass, parallel_thresh=sfdefaults.xenapi_parallel_calls_thresh, parallel_max=sfdefaults.xenapi_parallel_calls_max, debug=False):
        ...
        mylog.info(str(len(matched_vms.keys())) + " VMs will be reverted: " + ", ".join(matched_vms.keys()))
        ...
        if allgood:
            mylog.passed("Successfully reverted all VMs")
            return True
        else:
