"""
This script will start or stop vdbench on a group of xen clients
...
When run as a script, the following options/env variables apply:
    ...
    --start             The option to start vdbench

    --stop              The option to stop vdbench
...
class XenStartStopVdbenchAction(ActionBase):
    class Events:
        """
    ...
    def _StartVdBenchThread(self, ip, username, password, result, output=False):
        ...
        try:
            ...
            return
        retcode, stdout, stderr = client.ExecuteCommand("start vdbench")
        if retcode == 0:
            result[ip] = True
            if output:
                ...
                result[ip] = True
                mylog.info(ip + ": vdbench already running")
        else:
            mylog.error(ip + ": Could not start vdbench " + stderr)

    def _StopVdBenchThread(self, ip, username, password, result, output=False):
        client = libclient.SfClient()
        try:
            client.Connect(ip, username, password)
        except libclient.ClientError as e:
            mylog.error(ip + ": Could not connect. Message: " + str(e))
            return
        retcode, stdout, stderr = client.ExecuteCommand("stop vdbench")
        if retcode == 0:
            result[ip] = True
            if output:
                mylog.passed("\t" + ip + ": " + stdout[:-1])
                ...
                result[ip] = True
                mylog.info(ip + ": vdbench already stopped")
        else:
            mylog.error(ip + ": Could not stop vdbench " + stderr)
    ...
    def Execute(self, vm_regex=None, vmhost=None, host_user=sfdefaults.host_user, host_pass=sfdefaults.host_pass, clientUser=sfdefaults.client_user, clientPass=sfdefaults.client_pass, threadMax=10, start=False, stop=False, debug=False):
        ...
        for ip in ip_list:
            ...
            th = None
            if start:
                th = multiprocessing.Process(target=self._StartVdBenchThread, args=(ip, clientUser, clientPass, results, False))
            elif stop:
                th = multiprocessing.Process(target=self._StopVdBenchThread, args=(ip, clientUser, clientPass, results, False))
            if th is None:
        ...
        if allgood:
            if start:
                mylog.passed("All VMs started vdbench")
                return True
            elif stop:
                mylog.passed("All VMs stopped vdbench")
        else:
            if start:
                mylog.error("Not all VMs could start vdbench")
                return False
            elif stop:
                mylog.error("Not all VMs could stop vdbench")
                return False
