...
"""
This action will enable virtual disk serial numbers to be read inside a VM
...
class VmwareEnableVmDiskuuidAction(ActionBase):
    class Events:
    ...
    def Execute(self, vm_name, mgmt_server=sfdefaults.fc_mgmt_server, mgmt_user=sfdefaults.fc_vsphere_user, mgmt_pass=sfdefaults.fc_vsphere_pass, bash=False, csv=False, debug=False):
        ...
        try:
            with libvmware.VsphereConnection(mgmt_server, mgmt_user, mgmt_pass) as vsphere:
                ...
                libvmware.SetVMXProperty(vsphere, vm, 'disk.EnableUUID', 'true')
                mylog.passed("Successfully enabled disk UUIDs on " + vm_name)
