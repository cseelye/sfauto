...
if not new_hostname:
    ...
    if "vmware" in hypervisor or "esx" in hypervisor:
        try:
            ...
            mylog.info("Searching vCenter for a VM with my MAC address")
            vcenter = VIServer()
            vcenter.connect("vcenter.domain.local", "script_user", "password")
            try:
    ...
    if "kvm" in hypervisor:
        try:
            ...
            mylog.info("Searching KVM hosts for a VM with my MAC address")
